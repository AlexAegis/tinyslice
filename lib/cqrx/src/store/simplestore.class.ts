...
abstract class SimpleBaseStore<ParentState, Slice, Payload> extends Observable<Slice> {
	protected abstract state: BehaviorSubject<Slice>;
	protected abstract parent: SimpleBaseStore<unknown, ParentState, Payload> | undefined;
	...
	): SimpleStoreSlice<Slice, SubSlice, Payload>;
	...
	public addSlice<SubSlice, AdditionalKey extends string | number = never>( // TODO: must not already be present on slice
		...
		reducers: ReducerConfiguration<SubSlice, Payload>[] = [],
		comparator?: (a: SubSlice, b: SubSlice) => boolean
	): SimpleStoreSlice<Slice & Record<AdditionalKey, SubSlice>, SubSlice, Payload> {
		return new SimpleStoreSlice(
			this as unknown as SimpleBaseStore<
				unknown,
...
export class SimpleStore<State, Payload = any> extends SimpleBaseStore<unknown, State, Payload> {
	...
	#reducerRunner = SimpleStore.createReducerRunner(this.reducerConfigurations);
...
export class SimpleStoreSlice<ParentSlice, Slice, Payload> extends SimpleBaseStore<
	ParentSlice,
	Slice,
	Payload
> {
	...
	#reducerRunner = SimpleBaseStore.createReducerRunner(this.reducerConfigurations);

	constructor(
		protected readonly parent: SimpleBaseStore<unknown, ParentSlice, Payload>,
